...
public aspect UnsafeIteratorMonitorAspectOptimized {
	...
	static long max_create = 0, max_next = 0, max_update = 0;
	static long min_create = Long.MAX_VALUE, min_next = Long.MAX_VALUE, min_update = Long.MAX_VALUE;
	...
	after (Collection c) : UnsafeIterator_updatesource1(c) {
	    ...
	    if(duration > max_update && duration > 0)
	    {
	    	max_update = duration;
	    }

	    if(duration < min_update && duration > 0)
	    {
	    	min_update = duration;
	    }
	...
	void around(): System_exit()
	{
		...
		System.out.println("Max update : "+ max_update);
		System.out.println("Max next : "+ max_next);
		System.out.println("Min create : "+ min_create);
		System.out.println("Min update : "+ min_update);
